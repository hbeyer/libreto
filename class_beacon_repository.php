...
class beacon_repository {
    ...
    public $beacon_sources = array(
        ...
        'cors' => array('label' => 'correspSearch – Verzeichnisse von Briefeditionen', 'location' => 'http://correspsearch.bbaw.de/api/v1/gnd-beacon.xql?correspondent=all', 'target' => 'http://correspsearch.bbaw.de/search.xql?correspondent=http://d-nb.info/gnd/{ID}&l=de'),
...
// '' => array('label' => '', 'location' => '', 'target' => ''),
    ...
    function __construct() {
        ...
        foreach ($this->beacon_sources as $key => $source) {
            if (!file_exists($this->folder.'/'.$key)) {
				$this->missingFiles[] = $source['label'];
                unset($this->beacon_sources[$key]);
            }
        }        
    }
    ...
    private function makeLink($key, $gnd, $target = '_blank') {
